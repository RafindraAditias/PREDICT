...
# # CSS SAFE LOAD
# # =========================
# def inject_css_local():
#     st.markdown(
#         """
#         <style>
#           .block-container { padding-top: 3rem; padding-bottom: 2rem; max-width: 1400px; }
#           .stTabs [data-baseweb="tab-list"] { margin-top: 1rem; }
#           h1, h2, h3 { margin-top: 1.2rem !important; }
#           .muted { opacity: 0.75; }
#         </style>
#         """,
...
# with st.expander("Cek dataset (preview)", expanded=False):
#     st.write("5 baris pertama")
#     st.dataframe(df.head(), use_container_width=True)
#     st.write("5 baris terakhir")
...
# required_keys = {"forecast", "train", "test", "metrics"}
# missing = required_keys - set(result.keys())
# if missing:
#     st.error(
#         f"Output model kurang key: {missing}. Pastikan fungsi model return dict dengan key forecast, train, test, metrics."
#     )
#     st.stop()
...
# tabs = st.tabs(["Overview", "Forecasting", "Model Comparison", "Risk", "Forecast Table"])
...
#     comp = pd.DataFrame([sar["metrics"], ari["metrics"], ets["metrics"]])
...
# with tabs[3]:
#     st.subheader("Risk")
#     threshold = st.slider("Threshold (µg/m³)", 15, 120, 55, 5)
#     prob = float((forecast > float(threshold)).mean() * 100)
#     st.metric("Probability exceed threshold", f"{prob:.1f}%")

#     table = forecast_table(forecast)
#     table["Exceed"] = table["PM2.5 Forecast"] > float(threshold)
...
#     full_tbl = forecast_table(forecast)
#     st.dataframe(full_tbl, use_container_width=True)

#     csv_bytes = full_tbl.to_csv(index=False).encode("utf-8")
#     st.download_button("Download forecast CSV", csv_bytes, file_name="pm25_forecast.csv", mime="text/csv")
...
# =========================
# TAB 2 MODEL COMPARISON (PERBAIKAN UTAMA DI SINI!)
# =========================
with tabs[2]:
    st.subheader("Model Comparison (Locked ETS)")
